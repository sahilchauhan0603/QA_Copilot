...
const IntegrationSettings = () => {
  ...
  const [initialJiraForm, setInitialJiraForm] = useState({
    url: "",
    email: "",
    api_token: "",
  });
  ...
  const [zephyrForm, setZephyrForm] = useState({
    zephyr_token: "",
    project_key: "",
  });
  const [initialZephyrForm, setInitialZephyrForm] = useState({
    zephyr_token: "",
    project_key: "",
  });
  ...
  // TestRail form
  const [testrailForm, setTestrailForm] = useState({
    url: "",
    email: "",
    api_key: "",
    project_id: "",
  });
  const [initialTestrailForm, setInitialTestrailForm] = useState({
    url: "",
    email: "",
    api_key: "",
    project_id: "",
  ...
  const xrayHasChanges = xrayForm.project_key !== initialXrayForm.project_key;
  ...
  const xrayReady = !!xrayForm.project_key?.trim() && isConfigured("jira");
  ...
  const confirmDeleteIntegration = async () => {
    ...
    try {
      ...
      toast.success(
        `${integrationLabels[pendingDeleteType] || "Integration"} configuration removed`,
      );
  ...
  const handleAdoTest = async () => {
    ...
    try {
      const result = await integrationAPI.testConnection(
        ...
        {
          organization_url: adoForm.organization_url,
          project: adoForm.project,
        },
      );
  ...
  const handleZephyrSave = async () => {
    ...
    if (!isConfigured("jira")) {
      toast.error(
        "Jira must be configured first (Zephyr uses Jira credentials)",
      );
      return;
    }
  ...
  const handleTestrailTest = async () => {
    ...
    try {
      const result = await integrationAPI.testConnection(
        ...
        {
          url: testrailForm.url,
          email: testrailForm.email,
          project_id: testrailForm.project_id,
        },
      );
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700 mt-1">
            Integration credentials are scoped to your current workspace
            (personal or team). Configure ticket tracking integrations (Jira,
            Azure DevOps) and test management tools (Xray, Zephyr, TestRail) to
            automate your workflow. All API tokens and keys are encrypted at
            rest using AES-256.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-4 mt-5 pt-5 border-t border-gray-200">
            <div>
              ...
              <input
                ...
                value={jiraForm.url}
                onChange={(e) =>
                  setJiraForm((p) => ({ ...p, url: e.target.value }))
                }
                className="input"
          ...
          <div className="space-y-4 mt-5 pt-5 border-t border-gray-200">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAdoForm((p) => ({ ...p, project: e.target.value }))
                }
                className="input"
          ...
          <div className="space-y-4 mt-5 pt-5 border-t border-gray-200">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setXrayForm((p) => ({ ...p, project_key: e.target.value }))
                }
                className="input"
          ...
          <div className="space-y-4 mt-5 pt-5 border-t border-gray-200">
            {!isConfigured("jira") && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-start gap-2">
                ...
                />
                <p className="text-sm text-yellow-800">
                  Zephyr uses your Jira credentials. Please configure Jira
                  first.
                </p>
              </div>
            )}
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setZephyrForm((p) => ({ ...p, project_key: e.target.value }))
                }
                className="input"
                placeholder="e.g., PROJ"
              />
            ...
            <div>
              <label className="input-label">
                Zephyr API Token <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                <input
                  type="password"
                  ...
                  placeholder={
                    isConfigured("zephyr")
                      ? "****************"
                      : "Enter Zephyr Scale API token"
                  }
                />
              </div>
              <p className="input-hint">
                Required for first-time setup. Leave blank only if you already
                saved a token and are updating other fields.
              </p>
              <p className="input-hint">
                <a
                  ...
                  className="text-primary-600 hover:underline inline-flex items-center gap-1"
                >
                  Learn how to generate Zephyr API token{" "}
      ...
      <div className="card">
        ...
        {expandedSection === "testrail" && (
          <div className="space-y-4 mt-5 pt-5 border-t border-gray-200">
            <div>
              <label className="input-label">
                TestRail URL <span className="text-red-500">*</span>
              </label>
              <input
                type="url"
                value={testrailForm.url}
                onChange={(e) =>
                  setTestrailForm((p) => ({ ...p, url: e.target.value }))
                }
                className="input"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setTestrailForm((p) => ({ ...p, email: e.target.value }))
                }
                className="input"
                placeholder="your-email@company.com"
              />
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    setTestrailForm((p) => ({ ...p, api_key: e.target.value }))
                  }
                  className="input pr-10"
                  placeholder={
                    isConfigured("testrail")
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setTestrailForm((p) => ({ ...p, project_id: e.target.value }))
                }
                className="input"
                placeholder="e.g., 1"
              />
              <p className="input-hint">Project ID for test case exports</p>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                disabled={
                  testrailSaving || !testrailReady || !testrailHasChanges
                }
                className="btn-primary flex items-center gap-2 text-sm disabled:cursor-not-allowed disabled:opacity-50"
              >
                {testrailSaving ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      This will delete saved credentials
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="px-6 py-5">
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-semibold">
                    {integrationLabels[pendingDeleteType] || "this integration"}
                  </span>{" "}
                  configuration?
                </p>
            ...
            <div className="bg-white rounded-xl max-w-md w-full shadow-2xl">
              ...
              <div className="px-6 py-5 space-y-4">
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-start gap-2">
                  ...
                  />
                  <p className="text-sm text-yellow-800">
                    Enter your account password to decrypt and view the stored
                    credentials.
                  </p>
                </div>
                ...
                {!revealedToken ? (
                  <div>
                    ...
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handlePasswordVerify()
                      }
                      className="input"
