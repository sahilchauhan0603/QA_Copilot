...
import { testGenAPI, integrationAPI, testManagementAPI } from '../services/api';
...
const DetailViewModal = ({ selectedGeneration, onClose, onDownloadExcel, integrationConfigs = [] }) => {
  ...
  // Test management export state
  const [showExportMenu, setShowExportMenu] = useState(false);
  const [exporting, setExporting] = useState(null); // 'xray' | 'zephyr' | 'testrail' | null
  const [exportSuiteName, setExportSuiteName] = useState('');
  const [showExportDialog, setShowExportDialog] = useState(false);
  const [selectedExportTool, setSelectedExportTool] = useState(null);

  const handleExport = async (tool) => {
    if (!gen?.id) return;

    // TestRail requires suite name, others are optional
    if (tool === 'testrail' || !exportSuiteName) {
      setSelectedExportTool(tool);
      setShowExportDialog(true);
      setShowExportMenu(false);
      return;
    }

    await performExport(tool, exportSuiteName);
  };
  ...
  const performExport = async (tool, suiteName) => {
    ...
    try {
      let result;
      if (tool === 'xray') {
        result = await testManagementAPI.exportToXray(gen.id, suiteName || null, gen.ticket_id || null);
        toast.success(`Exported ${result.result.created} test cases to Xray`);
      } else if (tool === 'zephyr') {
        result = await testManagementAPI.exportToZephyr(gen.id, suiteName || null, gen.ticket_id || null);
        ...
        result = await testManagementAPI.exportToTestRail(gen.id, suiteName, gen.ticket_id || null);
  ...
  const handleRefineClick = (type) => {
    ...
    // Focus refinement needs additional input
    if (type === 'focus') {
  ...
  const performRefinement = async (type, options = {}) => {
    ...
    try {
      ...
      if (type === 'regenerate') {
        // Use SSE for regenerate
        result = await testGenAPI.refine(gen.id, type, options, (progressData) => {
          // Progress callback handled same as generation
          if (progressData.type === 'step') {
            toast(`${progressData.label} - ${progressData.status}`, { icon: '⚙️', duration: 1500 });
          }
        });
        toast.success('Test cases regenerated successfully!');
      } else {
        // Direct call for other refinements
        result = await testGenAPI.refine(gen.id, type, options);
      ...
      // Reload the page or update the UI with new generation
      if (result.generation) {
        // Close current modal and show notification to view new refined result
        toast.success(
        ...
        setTimeout(() => {
          onClose();
          // Trigger refresh of generations list if parent has refresh function
      ...
      <div className="bg-gray-50 rounded-xl max-w-6xl w-full max-h-[93vh] flex flex-col shadow-2xl">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 rounded-t-xl px-6 py-4 flex items-center justify-between z-10 shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative">
              ...
              {showRefineMenu && (
                <>
                  <div className="fixed inset-0 z-20" onClick={() => setShowRefineMenu(false)} />
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 rounded-b-xl px-6 py-3 flex items-center justify-between shrink-0">
          <div className="text-xs text-gray-400">
            ...
            {exporting && (
              <span className="ml-3 text-purple-600 font-medium inline-flex items-center gap-1">
                <Loader size={12} className="animate-spin" />
                {exporting === 'xray' ? 'Exporting to Xray...' : exporting === 'zephyr' ? 'Exporting to Zephyr...' : 'Exporting to TestRail...'}
              </span>
            )}
            {refining && (
              <span className="ml-3 text-indigo-600 font-medium inline-flex items-center gap-1">
                <Loader size={12} className="animate-spin" />
                {refinementType === 'regenerate' ? 'Regenerating test cases...' : 
                 refinementType === 'minimize' ? 'Minimizing test cases...' :
                 refinementType === 'focus' ? 'Generating focused tests...' :
                 refinementType === 'edge_cases' ? 'Adding edge cases...' :
                 refinementType === 'coverage' ? 'Improving coverage...' :
                 refinementType === 'simplify' ? 'Simplifying tests...' :
                 'Refining results...'}
              </span>
            )}
          </div>
          ...
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-50 rounded-xl">
            <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
              <h4 className="text-lg font-bold text-gray-900 mb-4">
                Export to {selectedExportTool === 'xray' ? 'Xray' : selectedExportTool === 'zephyr' ? 'Zephyr Scale' : 'TestRail'}
              </h4>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {selectedExportTool === 'zephyr' ? 'Test Cycle Name' : 'Test Suite Name'}
                  {selectedExportTool === 'testrail' && <span className="text-red-500">*</span>}
                </label>
                <input
                  type="text"
                  value={exportSuiteName}
                  onChange={(e) => setExportSuiteName(e.target.value)}
                  placeholder={`Enter ${selectedExportTool === 'zephyr' ? 'cycle' : 'suite'} name...`}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                  autoFocus
                />
                <p className="mt-2 text-xs text-gray-500">
                  {selectedExportTool === 'testrail' 
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  onClick={() => performExport(selectedExportTool, exportSuiteName)}
                  disabled={selectedExportTool === 'testrail' && !exportSuiteName.trim()}
        ...
        {/* ─── Refine Dialog (for Focus Area) ─── */}
        {showRefineDialog && refinementType === 'focus' && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-50 rounded-xl">
            <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
              <h4 className="text-lg font-bold text-gray-900 mb-4">
