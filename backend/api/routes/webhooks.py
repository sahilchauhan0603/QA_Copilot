...
@webhooks_bp.route('/recent-activity', methods=['GET'])
@token_required
def get_recent_activity(current_user):
    """Return recent auto-regenerated generations for the current workspace."""
    try:
        user_id = current_user['user_id']
        team_id = workspace_service.get_active_workspace(user_id)
        since_str = request.args.get('since')  # ISO-8601 timestamp

        db = get_db_connection()
        with db.get_session() as session:
            query = session.query(Generation).filter(
                Generation.user_id == user_id,
                Generation.generation_metadata['webhook_auto_regen'].astext == 'true',
            )
            if team_id is None:
            ...
            if since_str:
                try:
                    ...
                    query = query.filter(Generation.created_at > since_dt)
                except ValueError:
                    pass

            gens = query.order_by(Generation.created_at.desc()).limit(20).all()
            ...
            results = [
                {
                    ...
                    'auto_regen_source': (g.generation_metadata or {}).get('auto_regen_source'),
                    'timestamp': g.timestamp.isoformat() if g.timestamp else None,
                    'created_at': g.created_at.isoformat() if g.created_at else None,
                }
                for g in gens
            ]
            return jsonify({'activity': results, 'total': len(results)}), 200
    except Exception as e:
...
def _process_subscription(session, sub):
    ...
    try:
        ...
        # Mark this generation as auto-regenerated by webhook
        final_state['webhook_auto_regen'] = True
        final_state['auto_regen_source'] = integration_type
